// Created Cancel variable for using the icon
...
window.addEventListener("load", () => {
  ...
  //LocalStorageden görevleri al
  let tasks = JSON.parse(localStorage.getItem("tasks")) || [];

  // Mevcut görevleri sayfaya yükle
  tasks.forEach((task) => {
  ...
  form.addEventListener("submit", (e) => {
    ...
    tasks.push(task); // Görevi diziye ekle
    localStorage.setItem("tasks", JSON.stringify(tasks)); // LocalStorage'ı güncelle
    createTaskElement(task); // Görevi sayfaya ekle
  ...
  function createTaskElement(task) {
    ...
    // Görev tamamlandıysa çizgiyi çek
    if (task.completed) {
    ...
    task_cancel_icon.addEventListener("click", () => {
      if (confirm("Are you sure you want to delete this task?")) {
        ...
        tasks = tasks.filter((t) => t !== task); // Görevi diziden çıkar
        localStorage.setItem("tasks", JSON.stringify(tasks)); // LocalStorage'ı güncelle
      }
    });

    task_completed_button.addEventListener("click", () => {
      task.completed = !task.completed; // Tamamlanma durumunu değiştir
      if (task.completed) {
        task_input.style.textDecoration = "line-through";
      } else {
        task_input.style.textDecoration = "none";
